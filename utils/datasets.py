...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        bi = np.floor(np.arange(n) / batch_size).astype(np.intc)  # batch index
        ...
        if self.rect:
            ...
            self.batch_shapes = np.ceil(np.array(shapes) * img_size / stride + pad).astype(np.intc) * stride
            ...
            if l.shape[0]:
                ...
                if extract_bounding_boxes:
                    ...
                    for j, x in enumerate(l):
                        ...
                        b[2:] = b[2:] * 1.3 + 30  # pad
                        b = xywh2xyxy(b.reshape(-1, 4)).ravel().astype(np.intc)
...
def letterbox(img, new_shape=(608, 608), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
